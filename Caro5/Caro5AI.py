...
def is_win(board):
    ...
    black = score_of_col(board,'X')
    white = score_of_col(board,'O')
...
def click(x,y):
    ...
    if board[y][x] == ' ':
        ...
        if game_res in ["PC Win", "You Win", "Draw"]:
            print (game_res)
            win = True
            time.sleep(5)
            exit(0)
        ...
        ay,ax = best_move(board,'O')
        draw_stone(ax,ay,colors['O'])
        board[ay][ax]='O'    
...
def initialize(size):
    ...
    screen.setworldcoordinates(-1,size+0.6,size+0.6,-1)
    screen.bgcolor('white')
    screen.tracer(500)
    
    colors = {'O':turtle.Turtle(),'X':turtle.Turtle()}
    colors['O'].color('red')
    colors['X'].color('blue')
